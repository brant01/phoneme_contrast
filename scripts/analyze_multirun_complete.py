...
class CompleteMultirunAnalyzer:
    ...
    def analyze_all_runs(self):
        ...
        if not run_dirs:
            ...
            parent_runs = [
                d for d in self.multirun_dir.parent.iterdir() if d.is_dir() and d.name.isdigit()
            ]
            if parent_runs:
    ...
    def analyze_single_run(self, run_dir: Path) -> Dict:
        ...
        data = {"path": str(run_dir), "run_id": run_dir.name}
    ...
    def extract_embeddings_from_best(self):
        ...
        try:
            ...
            # If config is incomplete in checkpoint, load from hydra config file
            if not config or "model" not in config or not config["model"]:
                self.logger.info(
                    "Config incomplete in checkpoint, loading from hydra config file..."
                )
                hydra_config_path = Path(self.best_run["path"]) / ".hydra" / "config.yaml"
                if hydra_config_path.exists():
                    with open(hydra_config_path, "r") as f:
                        config = yaml.safe_load(f)
                        self.logger.info(f"Loaded config from {hydra_config_path}")
            ...
            data_path = Path(
                config.get("data", {}).get("data_path", "data/raw/New Stimuli 9-8-2024")
            )
            ...
            with torch.no_grad():
                for i, (file_path, label) in enumerate(zip(file_paths, labels)):
                    ...
                    # File paths contain "Male" or "Female" in the path
                    file_path_str = str(file_path)
                    if "Male" in file_path_str and "Female" not in file_path_str:
                        all_genders.append("M")
                    elif "Female" in file_path_str:
            ...
            self.logger.info(
                f"Samples per phoneme: min={np.bincount(self.labels).min()}, "
                f"max={np.bincount(self.labels).max()}"
            )
    ...
    def run_inference_for_embeddings(self, checkpoint_path: Path):
        ...
        try:
            ...
            for i, (file_path, label) in enumerate(zip(file_paths, labels)):
                ...
                # Extract MFCC features
                mfcc_transform = torchaudio.transforms.MFCC(
                    sample_rate=target_sr, n_mfcc=40, melkwargs={"n_fft": 400, "hop_length": 160}
                )
            ...
            for i in range(n_classes):
                ...
                for j in range(samples_per_class):
                    ...
                    self.genders.append(
                        "M" if j % 2 == 0 else "F"
                    )  # Alternate genders for synthetic data
    ...
    def plot_multirun_comparison(self):
        ...
        ax = axes[0, 0]
        scatter = ax.scatter(
            self.comparison_df["temperature"],
        ...
        pivot_table = self.comparison_df.pivot_table(
            values="best_rf_acc", index="learning_rate", columns="temperature", aggfunc="mean"
        )
        ...
        ax.axis("tight")
        ax.axis("off")
        summary_df = self.comparison_df[
            ["run_id", "temperature", "learning_rate", "best_rf_acc"]
        ].round(3)
        table = ax.table(
    ...
    def plot_tsne(self):
        ...
        embeddings_2d = tsne_2d.fit_transform(self.embeddings)

        # Create 2D plot with labels
        ...
        if len(unique_phonemes) > 20:
            ...
            colors = np.vstack(
                [colors, plt.cm.tab20b(np.linspace(0, 1, len(unique_phonemes) - 20))]
            )

        # Create a mapping from phoneme to color
        phoneme_to_color = {
            phoneme: colors[i % len(colors)] for i, phoneme in enumerate(unique_phonemes)
        }
        ...
        texts = []
        for idx in range(len(self.phonemes)):
            gender = self.genders[idx] if idx < len(self.genders) else "?"
            label_text = f"{self.phonemes[idx]} ({gender})"
            texts.append(
                ax.annotate(
                    label_text,
        ...
        self._adjust_text_positions(texts, embeddings_2d, ax)

        ax.set_title("Phoneme Embeddings t-SNE Visualization", fontsize=20)
        ...
        try:
            ...
            # Silhouette score requires at least 2 labels to be present
            self.logger.debug(f"Could not compute silhouette score: {e}")
        ...
        # Save embeddings with phoneme names and gender info
        np.savez(
    ...
    def _adjust_text_positions(self, texts, points, ax):
        """Adjust text positions to minimize overlap."""
        # Get display coordinates for all points and texts
        # Simple repulsion-based adjustment
        for i, text in enumerate(texts):
            # Get the position of this text
            x, y = points[i]

            # Check for nearby points
            min_dist = 0.05  # Minimum distance in data coordinates
            offset_scale = 0.02
            ...
            for j, other_point in enumerate(points):
                if i != j:
                    ...
                    if dist < min_dist and dist > 0:
                        ...
                        repel_x = (x - other_point[0]) / dist
                        repel_y = (y - other_point[1]) / dist
                        dx += repel_x * offset_scale
                        dy += repel_y * offset_scale

            # Apply offset
            text.set_position((x + dx, y + dy))

            # Add arrow if text is displaced significantly
    ...
    def plot_confusion_matrix(self):
        ...
        if min_samples == 1:
            ...
            self.logger.info(
                f"Overall {n_splits}-fold accuracy: {overall_accuracy:.3f} ± "
        ...
        plt.title(
            f"Phoneme Confusion Matrix\n(CV Accuracy: {overall_accuracy:.3f}, "
            f"Total samples: {len(self.embeddings)})",
            fontsize=16,
        )
        ...
        plt.title(
            f"Phoneme Confusion Matrix (Normalized)\n(CV Accuracy: {overall_accuracy:.3f})",
            fontsize=16,
        )
        plt.xlabel("Predicted Phoneme", fontsize=12)
        plt.ylabel("True Phoneme", fontsize=12)
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "confusion_matrix_normalized.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
        ...
        if confused_pairs:
            ...
            for pair in confused_pairs[:10]:
                self.logger.info(
                    f"  {pair['true']} -> {pair['predicted']}: {pair['count']} times "
                    f"({pair['percentage']:.1%})"
                )
        ...
        results = {
            ...
            "samples_per_phoneme": {
                phoneme: int(count) for phoneme, count in zip(unique_phonemes, counts)
            },  # Convert to int
    ...
    def analyze_distances(self):
        ...
        # Add statistics
        stats_text = (
            f"Intra-class: {np.mean(intra_distances):.3f} ± {np.std(intra_distances):.3f}\n"
        )
        stats_text += (
            f"Inter-class: {np.mean(inter_distances):.3f} ± {np.std(inter_distances):.3f}\n"
        )
    ...
    def plot_learning_curves(self):
        ...
        if "train_loss" in metrics:
            ...
            if "val_loss" in metrics:
                val_epochs = np.linspace(
                    0, len(metrics["train_loss"]) - 1, len(metrics["val_loss"])
                )
        ...
        if "val_rf_accuracy" in metrics:
            ax = axes[0, 1]
            val_epochs = np.linspace(
                0, len(metrics["train_loss"]) - 1, len(metrics["val_rf_accuracy"])
            )
        ...
        ax = axes[1, 1]
        ax.axis("off")
        config_text = "Best Run Configuration:\n\n"
...
"""
        ...
        try:
            ...
            self.logger.debug("Markdown package not installed, skipping HTML report generation")
