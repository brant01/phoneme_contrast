...
def main():
    ...
    parser.add_argument(
        "--run-id", type=int, default=4, help="Run ID to analyze (default: 4, the best run)"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    rf_acc, rf_lower, rf_upper = significance_tester.bootstrap_confidence_interval(y_test, rf_pred)
    ...
    logger.info(
        f"RF CV Stability: {stability_results['mean_accuracy']:.3f} ± {stability_results['std_accuracy']:.3f}"
    )
    ...
    if np.sum(valid_mask) > 10:
        ...
        logger.info(
            f"Gender classification accuracy: {gender_results.get('gender_classification_accuracy', 0):.3f}"
        )
        logger.info(
            f"Cross-gender accuracy (mean): {gender_results.get('cross_gender_accuracy_mean', 0):.3f}"
        )
    ...
    # Feature importance
    feature_scores = {
        feat: metrics.get("linear_separability", 0) for feat, metrics in feature_clustering.items()
    }

    fig3 = visualizer.plot_feature_importance(feature_scores, title="Phonetic Feature Separability")
    ...
    if "gender_invariance" in all_results:
        fig4 = visualizer.plot_speaker_invariance_results(
            all_results["gender_invariance"], title="Speaker Invariance Analysis"
        )
