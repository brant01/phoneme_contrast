...
def plot_confusion_matrix(embeddings, labels, label_map, output_dir):
    ...
    # Plot heatmap
    sns.heatmap(
        cm, annot=True, fmt="d", cmap="Blues", xticklabels=phoneme_names, yticklabels=phoneme_names
    )
    ...
    report = classification_report(
        labels, predictions, target_names=phoneme_names, output_dict=True
    )
...
def plot_tsne(embeddings, labels, metadata, label_map, output_dir):
    ...
    scatter1 = axes[0].scatter(
        embeddings_2d[:, 0], embeddings_2d[:, 1], c=labels, cmap="tab20", alpha=0.7
    )
    ...
    scatter2 = axes[1].scatter(
        embeddings_2d[:, 0], embeddings_2d[:, 1], c=gender_colors, cmap="viridis", alpha=0.7
    )
    ...
    legend_elements = [
        Patch(facecolor="purple", label="Male"),
    ...
    for i, label1 in enumerate(unique_labels):
        for j, label2 in enumerate(unique_labels):
            ...
            confusion_scores.append(
                {"phoneme1": phoneme1, "phoneme2": phoneme2, "distance": float(dist)}
            )
...
def main(cfg: DictConfig):
    ...
    output_dir = (
        Path(cfg.checkpoint_path).parent.parent
    ...
    for i, pair in enumerate(confusable_pairs[:5]):
        print(
            f"{i + 1}. {pair['phoneme1']} <-> {pair['phoneme2']} (distance: {pair['distance']:.3f})"
        )
