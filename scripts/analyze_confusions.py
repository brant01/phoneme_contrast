...
def analyze_confusions(confusion_json_path):
    ...
    if perfect_confusions:
        ...
        for conf in perfect_confusions:
            print(
                f"{conf['source']:6} → {conf['target']:6} ({conf['count']}/{conf['total']} = 100.0%)"
            )
    ...
    if high_confusions:
        ...
        for conf in high_confusions[:10]:
            print(
                f"{conf['source']:6} → {conf['target']:6} ({conf['count']}/{conf['total']} = {conf['error_rate'] * 100:.1f}%)"
            )
    ...
    if place_confusions:
        ...
        for conf in sorted(place_confusions, key=lambda x: x["error_rate"], reverse=True)[:5]:
            print(
                f"{conf['source']:6} → {conf['target']:6} ({conf['src_place']} → {conf['tgt_place']}, {conf['error_rate'] * 100:.1f}%)"
            )
