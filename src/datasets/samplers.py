...
class ContrastiveBatchSampler(Sampler[List[int]]):
    ...
    def __init__(
        ...
        logger = get_logger()
        logger.info(
            f"Class distribution: min={min(class_counts.values())}, max={max(class_counts.values())}"
        )
        ...
        if min_samples_to_exclude > 0:
            ...
            if excluded:
                logger.warning(
                    f"Excluded classes {sorted(excluded)} with < {min_samples_to_exclude} samples"
                )
        else:
            ...
            if undersampled:
                logger.info("Classes requiring oversampling (sampling with replacement):")
