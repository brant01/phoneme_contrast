...
class PhonemeContrastiveDataset(Dataset):
    ...
    def __init__(
        ...
        self.n_views = (
            config.get("contrastive", {}).get("views_per_sample", 2) if mode == "train" else 1
        )
        ...
        for view_idx in range(self.n_views):
            ...
            if self.mode == "train":
                view_waveform = self._augment_waveform(
                    view_waveform, seed=int(idx * 10000 + view_idx)
                )
            ...
            if self.mode == "train" and self.augmentation_pipeline is not None:
                features = self.augmentation_pipeline(features, seed=int(idx * 20000 + view_idx))
    ...
    def _load_waveform(self, idx: int) -> torch.Tensor:
        ...
        Returns:
            Preprocessed waveform tensor of shape [1, max_samples]
        """
        # Check cache first
    ...
    def _pad_or_trim(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            waveform: Input waveform tensor of arbitrary length

        Returns:
            Waveform tensor padded or trimmed to max_samples length
        """
