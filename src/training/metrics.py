...
def compute_embedding_metrics(embeddings: torch.Tensor, labels: torch.Tensor) -> Dict[str, float]:
    ...
    if "avg_positive_distance" in metrics and "avg_negative_distance" in metrics:
        metrics["separation_ratio"] = metrics["avg_negative_distance"] / (
...
def evaluate_clustering(
    embeddings: torch.Tensor, labels: torch.Tensor, n_neighbors: int = 5
) -> Dict[str, float]:
    """
    ...
    return {"knn_accuracy_mean": scores.mean(), "knn_accuracy_std": scores.std()}
