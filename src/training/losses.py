...
class SupervisedContrastiveLoss(nn.Module):
    ...
    def __init__(
        self, temperature: float = 0.07, base_temperature: float = 0.07, reduction: str = "mean"
    ):
        super().__init__()
    ...
    def forward(
        self, features: torch.Tensor, labels: torch.Tensor, mask: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
        Args:
