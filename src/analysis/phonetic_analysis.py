...
class PhoneticFeatureAnalyzer:
    ...
    def analyze_feature_clustering(
        ...
        for feature_name in features_to_analyze:
            ...
            feature_labels = np.array(
                [1 if phoneme in feature_phonemes else 0 for phoneme in phoneme_labels]
            )
    ...
    def analyze_feature_distances(
        ...
        for feature_name, feature_phonemes in self.PHONETIC_FEATURES.items():
            ...
            results[feature_name] = {
                ...
                "distance_ratio": np.mean(across_feature_dists)
                / (np.mean(within_feature_dists) + 1e-10),
            }
    ...
    def find_phoneme_clusters(
        ...
        feature_scores = {}
        for feature_name, feature_phonemes in self.PHONETIC_FEATURES.items():
            feature_labels = np.array([1 if p in feature_phonemes else 0 for p in phoneme_labels])
    ...
    def _calculate_feature_metrics(
        ...
        if len(positive_embeddings) > 1 and len(negative_embeddings) > 1:
            ...
            metrics["distance_ratio"] = np.mean(between_distances) / (
                np.mean(within_distances) + 1e-10
            )
    ...
    def _analyze_cluster_features(self, cluster_phonemes: List[str]) -> Dict[str, float]:
        ...
        # Sort by F1 score
        sorted_features = sorted(feature_scores.items(), key=lambda x: x[1]["f1"], reverse=True)
    ...
    def generate_feature_report(
        ...
        sorted_features = sorted(feature_scores.items(), key=lambda x: x[1]["nmi"], reverse=True)
